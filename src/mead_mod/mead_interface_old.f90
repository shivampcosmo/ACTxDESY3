...
function execute(block,config) result(status)
	...
	DO j=1,settings%nz
		...
		DO i=1,SIZE(k)
			plin=p_lin(k(i),cosi)        
			CALL halomod(k(i),z,p1h,p2h,pfull,plin,lut,cosi,umh,bth)
			DO kk=1,1000
	...
	status = datablock_put_double_grid(block,nl_power, "k_h", k_out, "z", z_out, "p_k", p_out,'p1h_k',p1h_out,'p2h_k',p2h_out,’um’,um_out,’bt’,bt_out)
...
MODULE MHM
  ...
  FUNCTION bt(k,z,lut,plin,cosm)
    ...
    frac=fdamp(z,cosm)
    IF(frac .NE. 0.) THEN
       p_2h=(1.-frac*(tanh(k*sigv/sqrt(ABS(frac))))**2.)**0.5
    ELSE
       p_2h=plin
    END IF
  ...
  FUNCTION u_m(k,z,lut,cosm)
    ...
    DO i=1,lut%n
       ...
       integrand(i)=((lut%rv(i))**3.)*(wk(i))
    END DO


    p_1h=integrand*Dv*4.*pi/3.
