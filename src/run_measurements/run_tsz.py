...
class Jack_shear_new(object):
    def __init__(self, conf, distance_file, ra_a, dec_a, ra_b, dec_b, hpix_a, hpix_b, w_a,w_b, e1_a, e2_a, e1_b, e2_b, k1 = None,k2 = None,   corr='GG',  number_of_cores=2, fact_dist=2,
        ...
        print (len(self.ra_s_1),len(self.dec_s_1),len(self.ra_s_2),len(self.dec_s_2))
        ...
        print (len(self.e1_a),len(self.e2_a),len(self.e1_b),len(self.e2_b))
        ...
        print (len(hpix_a),len(hpix_b),len(w_a),len(w_b))
        self.k1 = k1
        self.k2 = k2
        self.corr = corr
    ...
    def collect(self, pairs):
        ...
        for n in range(self.conf['nbins']):
            for jk1 in range(len(pairs[:, 0, 0, n])):
                ...
                #print DR, DD, RD
                if self.corr == 'GG' or self.corr == 'KG':
    ...
    def parallel(self, i, j):
        ...
        if self.corr == "KG":
            ...
            print (len(self.ra_s_1[mask]))
            #print (len(self.ra_s_1[mask]),len(self.e2_a[mask]))
            try:
            
                cat_a = treecorr.Catalog(ra=self.ra_s_1[mask], dec=self.dec_s_1[mask], g1=self.e1_a[mask], g2=self.e2_a[mask],w=self.weight_a[mask],ra_units='deg', dec_units='deg')
            except:
                cat_a = None
            #print (cat_a)
            mask = np.in1d(self.jk_b, j)
            print (len(self.ra_s_2[mask]))
            try:
            #if 1==1:
                cat_b = treecorr.Catalog(ra=self.ra_s_2[mask], dec=self.dec_s_2[mask], ra_units='deg', dec_units='deg',
                                     k=self.k2[mask])
            except:
                cat_b = None

            #print (len(self.ra_s_2[mask]),len(self.k2[mask]))
            #print (cat_b)
            if (cat_a == None) ^ (cat_b == None):
                pairs = [np.zeros(self.conf['nbins']), np.zeros(self.conf['nbins']), np.zeros(self.conf['nbins'])]
            else:
                print ('pre')
                kk = treecorr.KGCorrelation(self.conf)
                kk.process(cat_b, cat_a)
                print ('after')
                pairs =[kk.xi * kk.weight, kk.xi_im*kk.weight,kk.weight]
            return pairs
        if self.corr == 'GG':
            mask = np.in1d(self.jk_a, i)
            #try:
        ...
        if self.corr == 'KK':
            mask = np.in1d(self.jk_a, i)
            try:
                cat_a = treecorr.Catalog(ra=self.ra_s_1[mask], dec=self.dec_s_1[mask], ra_units='deg', dec_units='deg',
                                     k=self.k1[mask])
            except:
                cat_a = None
        ...
        def fun_speedup(othersample, othersample1, jackk):
            ...
            if os.path.exists(path+'.pkl'):
                ...
                try:
                    ...
                    self.pairs_ring[jackk][1] = [np.zeros(self.conf['nbins'])]*3
                    self.pairs_ring[jackk][0] = [np.zeros(self.conf['nbins'])]*3
            else:
         ...
         for mute_w in (np.unique(b[:, 1])):
          ...
          try:
              for prs in range(gm):
                ...
                self.pairs_ring[mute_w][1] = [np.zeros(self.conf['nbins'])]*3
                self.pairs_ring[mute_w][0] = [np.zeros(self.conf['nbins'])]*3
...
def run_MAP(i):
    ...
    rewrite = False
    runs_path = load_obj("runs_path_tsz")
    mute_dict = load_obj(runs_path[i])
    print ("loaded ")
   # print (len(mute_dict['k1']))
    print (len(mute_dict['g1_1']))
    if  ((rewrite) or (not os.path.exists(mute_dict['path_pkl']+"_tsz"+".pkl"))):
            ...
            try:
                print (len(mute_dict['g2_2']))
            except:
                mute_dict['g1_2']=np.arange(4)
                mute_dict['g2_2']=np.arange(4)
                
            try:
                print (len(mute_dict['k2']))
            except:
                mute_dict['k1']=0.
                mute_dict['k2']=0.
            J = Jack_shear_new(mute_dict['conf'],mute_dict['label_dist'],mute_dict['ra_1'],mute_dict['dec_1'],
            mute_dict['ra_2'],mute_dict['dec_2'],mute_dict['hpix1'],mute_dict['hpix2'],np.ones(len(mute_dict['w1'])),
            np.ones(len(mute_dict['w2'])),(mute_dict['g1_1']-np.mean(mute_dict['g1_1'])),mute_dict['g2_1']-np.mean(mute_dict['g2_1']),
            mute_dict['g1_2']-np.mean(mute_dict['g1_2']),mute_dict['g2_2']-np.mean(mute_dict['g2_2']), corr=mute_dict['type'],
            k1=mute_dict['k1'],k2=mute_dict['k2'],number_of_cores=10, centers= mute_dict['centers'], njk=mute_dict['n_jck'],cov = True,
            outp_fold=mute_dict['outp_fold'],label = mute_dict['label']+"_noweight")
            
            print (mute_dict['type'])
            #'''
            ...
            save_obj(mute_dict['path_pkl']+"_tsz",sys_xipxim_high)
            #'''
...
if __name__ == '__main__':
    ...
    for i in range(len(runsss)):
        ...
        print (runs_path[i])
        if  ((rewrite) or (not os.path.exists(mute_dict['path_pkl']+"_tsz"+".pkl"))):
            #if "0.2_1.3"  in mute_dict['path_pkl']:
               list_run.append(i)
