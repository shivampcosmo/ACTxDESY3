  ...
  !Accuracy parameter
    ...
    TYPE HM_tables
        ...
        REAL :: sigv, sigv100, c3, knl, rnl, neff, sig8z,bt
        INTEGER :: n
    END TYPE HM_tables
    ...
    !!JD end generalize to variable w
      !MARCO WAS HERE *******
      FUNCTION bt(k,z,lut,plin,cosm)

        REAL :: p_2h,bt
        REAL, INTENT(IN) :: k, plin, z
        REAL :: frac, q3, P13, P22, a, b, c, knl
  ...
  FUNCTION u_m(k,z,lut,cosm)
    ...
    REAL, ALLOCATABLE :: integrand(:), wk(:),u_m(:)
    ...
    ALLOCATE(integrand(lut%n),wk(lut%n),u_m(lut%n))
    ...
    !Only call eta once
    et=eta(z,lut,cosm) ! This is one of the HALO MODEL PAR
...
!    Print *, k, z, u_m(500), lut%rv(750),lut%c(750),lut%nu(750)
    ...
    SUBROUTINE halomod(k,z,p1h,p2h,pfull,plin,lut,cosm,umh,bth,g_array)
    ...
    IF(ALLOCATED(umh)) DEALLOCATE(umh)
    ALLOCATE(umh(lut%n))
    umh = 0.0

    IF(ALLOCATED(g_array)) DEALLOCATE(g_array)
    ALLOCATE(g_array(lut%n))
    g_array = 0.
    ...
    SUBROUTINE diocane()
    END SUBROUTINE diocane
    
    SUBROUTINE bth_print(k,z,lut,plin,cosm)
        ...
        lut%bt=3.!bt(k,z,lut,plin,cosm)

      END SUBROUTINE bth_print
