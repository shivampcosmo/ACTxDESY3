...
def parse_arguments():
    ...
    parser.add_argument('--rmax', type=float, default=30.0, help='rmax')
    parser.add_argument('--zmin', type=float, default=0.5, help='rmin')
...
if __name__ == "__main__":
    ...
    thvmin = (rmin/Dcom_z)*(180.*60./np.pi)
    thvmax = (rmax/Dcom_z)*(180.*60./np.pi)
    thvmean = 0.5 * (thvmin + thvmax)
    minrad = 0.2 * thvmean
    maxrad = 2.5 * thvmean
    # minrad = 0.1
    # maxrad = 250.0
    ...
    file_suffix_save = '_cat_' + str('void') + '_z_' + str(minz) + '_' + str(maxz) + '_R_' + str(rmin) + '_' + str(rmax) + '_' + 'dojk_' + str(do_jk) + '_njk_' + str(njk)  + '_' + 'desy3' + '_w' + str(int(put_weights_datapoints))
