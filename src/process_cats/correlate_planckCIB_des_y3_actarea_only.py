...
def parse_arguments():
    ...
    parser.add_argument('--map', required=True, type=str, help='map_type type')
    parser.add_argument('--beta', type=str, default='1p2')
    parser.add_argument('--dogg', type=bool, default=False, help='map_type type')
...
if __name__ == "__main__":
    ...
    save_filename_jk_obj = 'jkobj_DES_' + 'v_1_2_0_unblind' + '_' + '_njk_' + str(njk) + '_v16Jan21.pk'
    ...
    ydir = '/global/cfs/cdirs/des/shivamp/ACTxDESY3_data/planck_data/'
    # true_y_file = ydir + 'GNILC_CIB857_rotated_celestial_DES_coords.fits'
    if map_type == 'cib857':
        true_y_file = ydir + 'GNILC_CIB857_rotated_celestial_DES_coords.fits'
    if map_type == 'cib545':
        true_y_file = ydir + 'GNILC_CIB545_rotated_celestial_DES_coords.fits'
    if map_type == 'temp545':
        true_y_file = ydir + 'SkyMap_545_rotated_celestial_DES_coords.fits'
    if map_type == 'temp857':
        true_y_file = ydir + 'SkyMap_857_rotated_celestial_DES_coords.fits'
    ...
    ra_y, dec_y = ang2eq(pix_theta, pix_phi)
    # nside = hp.npix2nside(len(ymap_truth))
    # nside_ymap = nside
    # mask_final = []
    ydir = '/global/project/projectdirs/des/shivamp/ACTxDESY3_data/act_ymap_releases/deproj_cib_colin/'
    ...
    ind_datapoints = hp.ang2pix(nside, theta_datapoint_all, phi_datapoint_all)
    int_ind = np.in1d(ind_datapoints, ind_unmasked)
    selection_mask = np.where(int_ind == True)[0]

    rand_theta_all, rand_phi_all = eq2ang(rand_ra_all, rand_dec_all)
    ind_rand = hp.ang2pix(nside, rand_theta_all, rand_phi_all)
    int_ind_rand = np.in1d(ind_rand, ind_unmasked)
    selection_mask_rand = np.where(int_ind_rand == True)[0]

    del rand_theta_all, rand_phi_all, theta_datapoint_all, phi_datapoint_all
    ...
    for jz in range(len(zmin_bins)):
        ...
        # if map_type == 'cib_jch':
        #     file_suffix_save += '_betaCIB' + str(betacib)

        filename = save_dir + 'dy/dy_' + 'PL_map_' + str(map_type) + '_v1.2.0_' + 'wbeam' + '_pixell' + '_' + file_suffix_save + '_hrespixell_ACTareaonly_v16Feb21.pk'
        if not os.path.isfile(filename):
            ...
            save_data = {
                        ...
                        'xi_dy': xi_dy_full,'r_dy': r_dy, 'cov_dy': cov_dy,
